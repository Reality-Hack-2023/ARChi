...
static inline __attribute__((always_inline))
float ParamMapingOffset(thread const float& val, thread const float& offset)
{
    return (val + offset) - 0.5;
}

static inline __attribute__((always_inline))
float ParamMapingScaleDark(thread const float& val, thread const float& scale)
{
    return mix(1.0, val, scale);
}

static inline __attribute__((always_inline))
float3 InverseNormalY(thread float3& normal)
{
...
float3 DecodeNormal(thread float3& normalTS, thread const float& normalIntensity)
{
    ...
    float3 _282 = InverseNormalY(param);
    normalTS = _282;
    float3 param_1 = normalTS;
    float param_2 = normalIntensity;
    float3 _287 = NormalIntensity(param_1, param_2);
    normalTS = _287;
...
float3 SamplerEncodedPanoramic(thread const texture2d<float> panoramic, thread const sampler panoramicSmplr, thread const float3& dir, thread const float& rotation, thread const float& lod)
{
    ...
    float2 _1291 = GetPanoramicTexCoordsFromDir(param, param_1);
    float2 uv = _1291;
    float2 param_2 = uv;
    float param_3 = lod;
...
float3 Diffuse_OrenNayar(thread const SurfaceParams& S, thread const LightParams& L)
{
    ...
    if (Cosri >= 0.0)
    {
        _483 = 1.0 / fast::max(ndl, ndv);
    }
    else
    {
        _483 = 1.0;
    }
    float C2 = (((0.449999988079071044921875 * s2) / (s2 + 0.0900000035762786865234375)) * Cosri) * _483;
...
fragment main0_out main0(main0_in in [[stage_in]], constant buffer_t& buffer, texture2d<float> _AlbedoTexture [[texture(0)]], texture2d<float> _MRAOTexture [[texture(1)]], texture2d<float> _NormalTexture [[texture(2)]], texture2d<float> _AmbientTexture [[texture(3)]], sampler _AlbedoTextureSmplr [[sampler(0)]], sampler _MRAOTextureSmplr [[sampler(1)]], sampler _NormalTextureSmplr [[sampler(2)]], sampler _AmbientTextureSmplr [[sampler(3)]])
{
    main0_out out = {};
    float2 uv0 = in.v_uv0;
    ...
    float3 t_MaskTex = _MRAOTexture.sample(_MRAOTextureSmplr, uv0).xyz;
    float param_2 = t_MaskTex.x;
    float param_3 = buffer._Metallic;
    float metallic = ParamMapingOffset(param_2, param_3);
    float param_4 = t_MaskTex.y;
    float param_5 = buffer._Roughness;
    float roughness = ParamMapingOffset(param_4, param_5);
    float param_6 = t_MaskTex.z;
    float param_7 = buffer._Occlusion;
    float ao = ParamMapingScaleDark(param_6, param_7);
    float3x3 TBN = float3x3(float3(normalize(in.v_tDirWS)), float3(normalize(in.v_bDirWS)), float3(normalize(in.v_nDirWS)));
    float3 clearCoatNormal = float3(0.0, 0.0, 1.0);
    float3 normal = float3(0.0, 0.0, 1.0);
    float3 normalTS = _NormalTexture.sample(_NormalTextureSmplr, uv0).xyz;
    float3 param_8 = normalTS;
    float param_9 = buffer._NormalIntensity;
    float3 _2217 = DecodeNormal(param_8, param_9);
    clearCoatNormal = _2217;
    normal = _2217;
    clearCoatNormal = TBN * clearCoatNormal;
    normal = clearCoatNormal;
    float param_10 = envInt;
    float param_11 = envRot;
    float3 param_12 = albedo;
    float param_13 = opacity;
    float param_14 = cutoff;
    float3 param_15 = normal;
    float3 param_16 = clearCoatNormal;
    float param_17 = metallic;
    float param_18 = roughness;
    float param_19 = ao;
    float subsurface;
    float param_20 = subsurface;
    float3 subsurfaceCol;
    float3 param_21 = subsurfaceCol;
    float3 subsurfaceColMultiply;
    float3 param_22 = subsurfaceColMultiply;
    float ior;
    float param_23 = ior;
    float transmittance;
    float param_24 = transmittance;
    float transmittanceColorAtDistance;
    float param_25 = transmittanceColorAtDistance;
    float thin;
    float param_26 = thin;
    float clearCoat;
    float param_27 = clearCoat;
    float clearCoatRoughness;
    float param_28 = clearCoatRoughness;
    float3 emissive;
    float3 param_29 = emissive;
    float anisotropic;
    float param_30 = anisotropic;
    float anisotropicRotate;
    float param_31 = anisotropicRotate;
    float rampID;
    float param_32 = rampID;
    float rim;
    float param_33 = rim;
    float3 rimCol;
    float3 param_34 = rimCol;
    float3 ambient;
    float3 param_35 = ambient;
    float3 matcap;
    float3 param_36 = matcap;
    float smoothFactor;
    float param_37 = smoothFactor;
    float4 finalColor = MainEntry(_AmbientTexture, _AmbientTextureSmplr, param_10, param_11, param_12, param_13, param_14, param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23, param_24, param_25, param_26, param_27, param_28, param_29, param_30, param_31, param_32, param_33, param_34, param_35, param_36, param_37, in.v_posWS, in.v_nDirWS, in.v_tDirWS, in.v_bDirWS, buffer.u_WorldSpaceCameraPos, buffer.u_DirLightsEnabled, buffer.u_DirLightNum, buffer.u_DirLightsDirection, buffer.u_DirLightsColor, buffer.u_DirLightsIntensity, buffer.u_PointLightsEnabled, buffer.u_PointLightNum, buffer.u_PointLightsPosition, buffer.u_PointLightsColor, buffer.u_PointLightsIntensity, buffer.u_PointLightsAttenRangeInv, buffer.u_SpotLightsEnabled, buffer.u_SpotLightNum, buffer.u_SpotLightsPosition, buffer.u_SpotLightsColor, buffer.u_SpotLightsIntensity, buffer.u_SpotLightsAttenRangeInv, buffer.u_SpotLightsDirection, buffer.u_SpotLightsOuterAngleCos, buffer.u_SpotLightsInnerAngleCos);
    ...
    float2 param_39 = screen_coord;
    out.glResult = ApplyBlendMode(param_38, param_39);
    return out;
}
