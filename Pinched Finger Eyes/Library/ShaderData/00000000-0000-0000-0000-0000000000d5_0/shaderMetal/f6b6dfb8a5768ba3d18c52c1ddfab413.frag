...
fragment main0_out main0(main0_in in [[stage_in]], constant buffer_t& buffer, texture2d<float> _MainTex [[texture(0)]], sampler _MainTexSmplr [[sampler(0)]])
{
    ...
    float3 _222 = result.xyz * pow(2.0, buffer._Exposure * alpha);
    result = float4(_222.x, _222.y, _222.z, result.w);
    float3 _232 = result.xyz + float3(buffer._Brightness * alpha);
    result = float4(_232.x, _232.y, _232.z, result.w);
    float3 param = result.xyz;
    float3 colorLMS = LinearToLMS(param);
    float param_1 = buffer._Temperature * alpha;
    float param_2 = buffer._Tint * alpha;
    colorLMS *= WhiteBalanceToLMSCoeffs(param_1, param_2);
    float3 param_3 = colorLMS;
    float3 _255 = LMSToLinear(param_3);
    result = float4(_255.x, _255.y, _255.z, result.w);
    float midpoint = 0.21763764321804046630859375;
    float3 _273 = ((result.xyz - float3(midpoint)) * ((buffer._Contrast * alpha) + 1.0)) + float3(midpoint);
    result = float4(_273.x, _273.y, _273.z, result.w);
    ...
    float3 _294 = mix(grayscale, result.xyz, float3(1.0 + (buffer._Saturation * alpha)));
    result = float4(_294.x, _294.y, _294.z, result.w);
