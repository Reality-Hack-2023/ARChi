...
void main()
{
    ...
    emissive = (texture(_EmissiveTexture, uv).xyz * _EmissiveIntensity) * _EmissiveColor.xyz;
    vec4 result = vec4(ambient, 1.0);
    vec3 param = color;
    vec3 param_1 = normal;
    vec3 param_2 = v_posWS;
    vec3 param_3 = emissive;
    vec3 _1662 = result.xyz + MainEntry(param, param_1, param_2, param_3).xyz;
    result = vec4(_1662.x, _1662.y, _1662.z, result.w);
    o_fragColor = result;
}
