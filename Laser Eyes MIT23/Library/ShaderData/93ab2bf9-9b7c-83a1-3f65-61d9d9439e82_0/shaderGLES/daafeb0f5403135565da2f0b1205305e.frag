...
vec3 SamplerEncodedPanoramic(mediump sampler2D panoramic, vec3 dir, float rotation, float lod)
{
    ...
    vec2 _1331 = GetPanoramicTexCoordsFromDir(param, param_1);
    vec2 uv = _1331;
    vec2 param_2 = uv;
    float param_3 = lod;
...
vec3 BlendOverlay(vec3 base, vec3 blend)
{
    ...
    return vec3(BlendOverlay(param, param_1), BlendOverlay(param_2, param_3), BlendOverlay(param_4, param_5));
}
...
vec4 ApplyBlendMode(vec4 color, vec2 uv)
{
    ...
    float param_3 = ret.w;
    vec3 _344 = BlendOverlay(param_1, param_2, param_3);
    ret = vec4(_344.x, _344.y, _344.z, ret.w);
    return ret;
}
