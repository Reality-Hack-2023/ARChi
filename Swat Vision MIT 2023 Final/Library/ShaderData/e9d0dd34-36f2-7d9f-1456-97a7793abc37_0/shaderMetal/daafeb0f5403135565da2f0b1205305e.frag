...
float3 SamplerEncodedPanoramic(thread const texture2d<float> panoramic, thread const sampler panoramicSmplr, thread const float3& dir, thread const float& rotation, thread const float& lod)
{
    ...
    float2 _1292 = GetPanoramicTexCoordsFromDir(param, param_1);
    float2 uv = _1292;
    float2 param_2 = uv;
    float param_3 = lod;
...
float3 Diffuse_OrenNayar(thread const SurfaceParams& S, thread const LightParams& L)
{
    ...
    if (Cosri >= 0.0)
    {
        _484 = 1.0 / fast::max(ndl, ndv);
    }
    else
    {
        _484 = 1.0;
    }
    float C2 = (((0.449999988079071044921875 * s2) / (s2 + 0.0900000035762786865234375)) * Cosri) * _484;
...
float4 ApplyBlendMode(thread const float4& color, thread const float2& uv, texture2d<float> u_FBOTexture, sampler u_FBOTextureSmplr)
{
    ...
    float param_3 = ret.w;
    float3 _300 = BlendScreen(param_1, param_2, param_3);
    ret = float4(_300.x, _300.y, _300.z, ret.w);
    return ret;
}

fragment main0_out main0(main0_in in [[stage_in]], constant buffer_t& buffer, texture2d<float> u_FBOTexture [[texture(0)]], texture2d<float> _AlbedoTexture [[texture(1)]], texture2d<float> _AmbientTexture [[texture(2)]], sampler u_FBOTextureSmplr [[sampler(0)]], sampler _AlbedoTextureSmplr [[sampler(1)]], sampler _AmbientTextureSmplr [[sampler(2)]])
{
    main0_out out = {};
    float2 uv0 = in.v_uv0;
    ...
    float param_2 = envInt;
    float param_3 = envRot;
    float3 param_4 = albedo;
    float param_5 = opacity;
    float param_6 = cutoff;
    float3 param_7 = normal;
    float3 param_8 = clearCoatNormal;
    float param_9 = metallic;
    float param_10 = roughness;
    float param_11 = ao;
    float subsurface;
    float param_12 = subsurface;
    float3 subsurfaceCol;
    float3 param_13 = subsurfaceCol;
    float3 subsurfaceColMultiply;
    float3 param_14 = subsurfaceColMultiply;
    float ior;
    float param_15 = ior;
    float transmittance;
    float param_16 = transmittance;
    float transmittanceColorAtDistance;
    float param_17 = transmittanceColorAtDistance;
    float thin;
    float param_18 = thin;
    float clearCoat;
    float param_19 = clearCoat;
    float clearCoatRoughness;
    float param_20 = clearCoatRoughness;
    float3 emissive;
    float3 param_21 = emissive;
    float anisotropic;
    float param_22 = anisotropic;
    float anisotropicRotate;
    float param_23 = anisotropicRotate;
    float rampID;
    float param_24 = rampID;
    float rim;
    float param_25 = rim;
    float3 rimCol;
    float3 param_26 = rimCol;
    float3 ambient;
    float3 param_27 = ambient;
    float3 matcap;
    float3 param_28 = matcap;
    float smoothFactor;
    float param_29 = smoothFactor;
    float4 finalColor = MainEntry(_AmbientTexture, _AmbientTextureSmplr, param_2, param_3, param_4, param_5, param_6, param_7, param_8, param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16, param_17, param_18, param_19, param_20, param_21, param_22, param_23, param_24, param_25, param_26, param_27, param_28, param_29, in.v_posWS, in.v_nDirWS, in.v_tDirWS, in.v_bDirWS, buffer.u_WorldSpaceCameraPos, buffer.u_DirLightsEnabled, buffer.u_DirLightNum, buffer.u_DirLightsDirection, buffer.u_DirLightsColor, buffer.u_DirLightsIntensity, buffer.u_PointLightsEnabled, buffer.u_PointLightNum, buffer.u_PointLightsPosition, buffer.u_PointLightsColor, buffer.u_PointLightsIntensity, buffer.u_PointLightsAttenRangeInv, buffer.u_SpotLightsEnabled, buffer.u_SpotLightNum, buffer.u_SpotLightsPosition, buffer.u_SpotLightsColor, buffer.u_SpotLightsIntensity, buffer.u_SpotLightsAttenRangeInv, buffer.u_SpotLightsDirection, buffer.u_SpotLightsOuterAngleCos, buffer.u_SpotLightsInnerAngleCos);
    ...
    float2 param_31 = screen_coord;
    out.glResult = ApplyBlendMode(param_30, param_31, u_FBOTexture, u_FBOTextureSmplr);
    return out;
}
