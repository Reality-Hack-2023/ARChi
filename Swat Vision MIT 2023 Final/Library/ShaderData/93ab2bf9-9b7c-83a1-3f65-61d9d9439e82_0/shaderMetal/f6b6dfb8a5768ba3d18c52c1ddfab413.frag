...
float3 SamplerEncodedPanoramic(thread const texture2d<float> panoramic, thread const sampler panoramicSmplr, thread const float3& dir, thread const float& rotation, thread const float& lod)
{
    ...
    float2 _1331 = GetPanoramicTexCoordsFromDir(param, param_1);
    float2 uv = _1331;
    float2 param_2 = uv;
    float param_3 = lod;
...
float3 Diffuse_OrenNayar(thread const SurfaceParams& S, thread const LightParams& L)
{
    ...
    if (Cosri >= 0.0)
    {
        _523 = 1.0 / fast::max(ndl, ndv);
    }
    else
    {
        _523 = 1.0;
    }
    float C2 = (((0.449999988079071044921875 * s2) / (s2 + 0.0900000035762786865234375)) * Cosri) * _523;
...
float BlendOverlay(thread const float& base, thread const float& blend)
{
    ...
    {
        _269 = 1.0 - ((2.0 * (1.0 - base)) * (1.0 - blend));
    }
    return _269;
}
...
float4 ApplyBlendMode(thread const float4& color, thread const float2& uv, texture2d<float> u_FBOTexture, sampler u_FBOTextureSmplr)
{
    ...
    float param_3 = ret.w;
    float3 _344 = BlendOverlay(param_1, param_2, param_3);
    ret = float4(_344.x, _344.y, _344.z, ret.w);
    return ret;
}

fragment main0_out main0(main0_in in [[stage_in]], constant buffer_t& buffer, texture2d<float> u_FBOTexture [[texture(0)]], texture2d<float> _AmbientTexture [[texture(1)]], sampler u_FBOTextureSmplr [[sampler(0)]], sampler _AmbientTextureSmplr [[sampler(1)]])
{
    main0_out out = {};
    float2 uv0 = in.v_uv0;
    ...
    out.glResult = ApplyBlendMode(param_29, param_30, u_FBOTexture, u_FBOTextureSmplr);
    return out;
}
