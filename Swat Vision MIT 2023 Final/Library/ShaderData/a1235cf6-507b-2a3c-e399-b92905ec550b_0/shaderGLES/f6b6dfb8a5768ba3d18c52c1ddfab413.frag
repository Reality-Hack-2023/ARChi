...
vec3 SamplerEncodedPanoramic(mediump sampler2D panoramic, vec3 dir, float rotation, float lod)
{
    ...
    vec2 _1316 = GetPanoramicTexCoordsFromDir(param, param_1);
    vec2 uv = _1316;
    vec2 param_2 = uv;
    float param_3 = lod;
...
vec3 BlendLighten(vec3 base, vec3 blend)
{
    ...
    return vec3(BlendLighten(param, param_1), BlendLighten(param_2, param_3), BlendLighten(param_4, param_5));
}
...
vec4 ApplyBlendMode(vec4 color, vec2 uv)
{
    ...
    float param_3 = ret.w;
    vec3 _326 = BlendLighten(param_1, param_2, param_3);
    ret = vec4(_326.x, _326.y, _326.z, ret.w);
    return ret;
}
