...
float3 DecodeNormal(thread float3& normalTS, thread const float& normalIntensity)
{
    ...
    float3 _273 = NormalIntensity(param, param_1);
    normalTS = _273;
...
float3 SamplerEncodedPanoramic(thread const texture2d<float> panoramic, thread const sampler panoramicSmplr, thread const float3& dir, thread const float& rotation, thread const float& lod)
{
    ...
    float2 _1278 = GetPanoramicTexCoordsFromDir(param, param_1);
    float2 uv = _1278;
    float2 param_2 = uv;
    float param_3 = lod;
...
float3 Diffuse_OrenNayar(thread const SurfaceParams& S, thread const LightParams& L)
{
    ...
    if (Cosri >= 0.0)
    {
        _470 = 1.0 / fast::max(ndl, ndv);
    }
    else
    {
        _470 = 1.0;
    }
    float C2 = (((0.449999988079071044921875 * s2) / (s2 + 0.0900000035762786865234375)) * Cosri) * _470;
...
fragment main0_out main0(main0_in in [[stage_in]], constant buffer_t& buffer, texture2d<float> _AlbedoTexture [[texture(0)]], texture2d<float> _MRAOTexture [[texture(1)]], texture2d<float> _NormalTexture [[texture(2)]], texture2d<float> _AmbientTexture [[texture(3)]], sampler _AlbedoTextureSmplr [[sampler(0)]], sampler _MRAOTextureSmplr [[sampler(1)]], sampler _NormalTextureSmplr [[sampler(2)]], sampler _AmbientTextureSmplr [[sampler(3)]])
{
    ...
    float3 _2201 = DecodeNormal(param_8, param_9);
    clearCoatNormal = _2201;
    normal = _2201;
